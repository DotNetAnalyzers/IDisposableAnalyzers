namespace ValidCode.NetCore
{
    ...
    public class Issue242
    {
        ...
        {
            var taskFromResultTrue = TaskFromResultTrue(); // No IDISP001 = OK (maybe some Task not awaited warning but that is out of the scope of this package)
            var b1 = await taskFromResultTrue;
            var b2 = await taskFromResultTrue.ConfigureAwait(false);
            var b3 = await TaskFromResultTrue();
            var b4 = await TaskFromResultTrue().ConfigureAwait(true);
            
            var taskOfMemoryStream1 = TaskTaskFromResultNewMemoryStream(); // No IDISP001 = OK (maybe some Task not awaited warning but that is out of the scope of this package)
            using var memoryStream1 = await taskOfMemoryStream1;
            var taskOfMemoryStream2 = TaskTaskFromResultNewMemoryStream();
            using var memoryStream2 = await taskOfMemoryStream2.ConfigureAwait(true);
            using var memoryStream3 = await TaskTaskFromResultNewMemoryStream(); // IDISP001 Generated = OK
            using var memoryStream4 = await TaskTaskFromResultNewMemoryStream().ConfigureAwait(true);


            var taskTaskFromResultHttpResponseMessage1 = TaskTaskFromResultHttpResponseMessage(); // No IDISP001 = OK (maybe some Task not awaited warning but that is out of the scope of this package)
            using var httpResponseMessage1 = await taskTaskFromResultHttpResponseMessage1;
            var taskTaskFromResultHttpResponseMessage2 = TaskTaskFromResultHttpResponseMessage();
            using var httpResponseMessage2 = await taskTaskFromResultHttpResponseMessage2.ConfigureAwait(true);
            using var httpResponseMessage3 = await TaskTaskFromResultHttpResponseMessage(); // IDISP001 Generated = OK
            using var httpResponseMessage4 = await TaskTaskFromResultHttpResponseMessage().ConfigureAwait(true);
