...
namespace N
{
    ...
    internal class C
    {
        internal C(IDisposable disposable)
        {
                ...
                Assert.AreEqual(false, DisposableWalker.Stores(localOrParameter, semanticModel, CancellationToken.None, out _));
            }
    ...
    internal class C
    {
        ...
        internal C(IDisposable disposable)
        {
            this.disposables.Add(disposable);
                ...
                Assert.AreEqual(true, semanticModel.TryGetSymbol(value, CancellationToken.None, out var symbol));
                Assert.AreEqual(true, LocalOrParameter.TryCreate(symbol, out var localOrParameter));
                Assert.AreEqual(true, DisposableWalker.Stores(localOrParameter, semanticModel, CancellationToken.None, out var container));
    ...
    public class C
    {
        ...
        public C(Stream stream)
        {
            this.disposable = new BinaryReader(stream);
                ...
                Assert.AreEqual(true, semanticModel.TryGetSymbol(value, CancellationToken.None, out var symbol));
                Assert.AreEqual(true, LocalOrParameter.TryCreate(symbol, out var localOrParameter));
                Assert.AreEqual(stores, DisposableWalker.Stores(localOrParameter, semanticModel, CancellationToken.None, out var container));
                Assert.AreEqual(stores, DisposableWalker.DisposedByReturnValue(syntaxTree.FindArgument("stream"), semanticModel, CancellationToken.None, out _));
                if (stores)
                {
    ...
    public class C
    {
        ...
        public C(HttpClientHandler handler)
        {
            this.disposable = new HttpClient(handler);
                ...
                Assert.AreEqual(true,   LocalOrParameter.TryCreate(symbol, out var localOrParameter));
                Assert.AreEqual(stores, DisposableWalker.Stores(localOrParameter, semanticModel, CancellationToken.None, out var container));
                Assert.AreEqual(stores, DisposableWalker.DisposedByReturnValue(syntaxTree.FindArgument("handler"), semanticModel, CancellationToken.None, out _));
                if (stores)
                {
                    Assert.AreEqual("N.C.disposable", container.ToString());
                }
