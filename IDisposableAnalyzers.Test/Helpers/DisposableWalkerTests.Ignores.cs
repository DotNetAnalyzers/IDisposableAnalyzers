    ...
    public class C
    {
        public static void M(string fileName)
        {
            var value = File.OpenRead(fileName);
                ...
                Assert.AreEqual(false, DisposableWalker.Ignores(value, semanticModel, CancellationToken.None, null));
            }
...
namespace N
{
    ...
    public static class C
    {
        public static object M() => string.Format(""{0}"", File.OpenRead(fileName));
                ...
                var semanticModel = compilation.GetSemanticModel(syntaxTree);
                var value = syntaxTree.FindExpression("File.OpenRead(fileName)");
                Assert.AreEqual(true, DisposableWalker.Ignores(value, semanticModel, CancellationToken.None, null));
            }
